...
template <typename Dtype>
void QuadrupletLossLayer<Dtype>::Forward_gpu(
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    ...
    Dtype mdist1 = std::max(margin1 + dist_ij - dist_ik, Dtype(0.0));
    	...
    	//prepare for backward pass
			caffe_gpu_set(1, Dtype(0), bottom[0]->mutable_gpu_diff() + (i*channels));
  		caffe_gpu_set(1, Dtype(0), bottom[1]->mutable_gpu_diff() + (i*channels));
    }
		else
		{
			caffe_gpu_set(1, Dtype(1), bottom[0]->mutable_gpu_diff() + (i*channels));
  		caffe_gpu_set(1, Dtype(-1), bottom[1]->mutable_gpu_diff() + (i*channels));
		}
		if(mdist2==Dtype(0)){
    	//dist_binary_.mutable_cpu_data()[i] = Dtype(0);
    	//prepare for backward pass
	//		bottom[0]->mutable_gpu_diff()[i*channels]=Dtype(0)+ bottom[0]->mutable_gpu_diff()[i*channels];
		//	 bottom[2]->mutable_gpu_diff()[i*channels]= Dtype(0);
			caffe_gpu_set(1, Dtype(0), bottom[2]->mutable_gpu_diff()+i*channels);
    }
		else
		{
			caffe_gpu_add_scalar(1,Dtype(1),bottom[0]->mutable_gpu_diff()+i*channels);
			caffe_gpu_set(1, Dtype(-1),bottom[2]->mutable_gpu_diff()+i*channels);
			//bottom[0]->mutable_gpu_diff()[i*channels] = Dtype(1)+ bottom[0]->mutable_gpu_diff()[i*channels];
			//bottom[2]->mutable_gpu_diff()[i*channels] = Dtype(-1);
		}
...
template <typename Dtype>
void QuadrupletLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
    const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  //Dtype margin = this->layer_param_.contrastive_loss_param().margin();
	int count = bottom[0]->count();
  for (int i = 0; i < 3; ++i) {
      Dtype alpha=top[0]->diff_at(0,0,0,0)/static_cast<Dtype>(bottom[i]->num());
		caffe_gpu_axpby(count, alpha, bottom[i]->mutable_gpu_diff(), Dtype(0.0), bottom[i]->mutable_gpu_diff());
  } //for i
}

INSTANTIATE_LAYER_GPU_FUNCS(QuadrupletLossLayer);
