			...
			if(bottom[1]->data_at(j,0,0,0)==bottom[1]->data_at(i,0,0,0) && i!=j)  //same class
			{
				for(int k=0; k<num; k++)
				{
					if(bottom[1]->data_at(k,0,0,0)!=bottom[1]->data_at(i,0,0,0))
					{
						...
						{
							Dtype weight = 3.6067*log(mdist1/margin1+1)+0.5;
							caffe_gpu_add_scalar(1,weight, bottom[0]->mutable_gpu_diff()+(i*num+j)*channels );
							caffe_gpu_add_scalar(1,Dtype(-1)*weight, bottom[0]->mutable_gpu_diff()+(i*num+k)*channels );
							term_num_++;
						}
						loss +=mdist1;	
