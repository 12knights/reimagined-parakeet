			...
			if(bottom[1]->data_at(j,0,0,0)==bottom[1]->data_at(i,0,0,0) && i!=j)  //same class
			{
				for(int k=0; k<num; k++)
				{
					if(bottom[1]->data_at(k,0,0,0)!=bottom[1]->data_at(i,0,0,0))
					{
						...
						{
							Dtype weight = 3.6067*log(mdist1+1)+0.5;//Dtype weight =7.0692*log(mdist1/margin1+1)+0.1;
							//Dtype weight = std::exp(1.3737*mdist1/margin1)-0.95;
							caffe_gpu_add_scalar(1,weight, bp_w+(i*num+j));
							caffe_gpu_add_scalar(1,weight, bp_w+(i*num+k));    
							term_num_++;
						}
						loss +=mdist1;	
...
void TripletSoftmaxLossLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
	...
	{
		for(int k=0; k<15; k++)
			LOG(INFO)<<bottom[0]->diff_at(i*num+k,0,0,0)<<",   "<<bottom[0]->diff_at(i*num+k,1,0,0);
		LOG(INFO)<<"                                      ";
	}
}
}
