...
template <typename Dtype>
 class HardQuadrupletLossLayer : public LossLayer<Dtype> {
  public:
   explicit HardQuadrupletLossLayer(const LayerParameter& param)
       : LossLayer<Dtype>(param){}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
       const vector<Blob<Dtype>*>& top);
 
   virtual inline int ExactNumBottomBlobs() const { return 2; }
   virtual inline const char* type() const { return "HardQuadrupletLoss"; }
   /**
    * Unlike most loss layers, in the TripletLossLayer we can backpropagate
    * to the first three inputs.
    */
...
template <typename Dtype>
 class TripletSoftmaxLossLayer : public LossLayer<Dtype> {
  public:
   explicit TripletSoftmaxLossLayer(const LayerParameter& param)
       : LossLayer<Dtype>(param){}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
       const vector<Blob<Dtype>*>& top);
  virtual inline int MinTopBlobs() const { return 1; }
  virtual inline int MaxTopBlobs() const { return 2; }
   virtual inline int ExactNumBottomBlobs() const { return 2; }
   virtual inline const char* type() const { return "TripletSoftmaxLossLayer"; }
   /**
    * Unlike most loss layers, in the TripletLossLayer we can backpropagate
    * to the first three inputs.
    */
